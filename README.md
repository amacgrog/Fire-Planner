...
- **First-run banner**: since the page loads with a fully-computed projection from generic example numbers (there's no persistence between visits, so every load looks the same), a dismissible callout above the results spells out that these aren't your numbers yet and points at the two ways to make them yours — the setup wizard, or importing a previously-exported scenario file. It clears itself the moment either happens, or the moment you edit any sidebar field directly
- **"Explore a question" menu**: a row of common questions above the results — retire earlier or later, what if a spouse dies first, what's actually driving the results, what if returns are worse than expected — each pre-wired to an existing comparison rather than a blank slate. The first two run the current plan against 2-3 alternatives and answer with a plain-language sentence (e.g. "Retiring 3 years earlier costs you $340K and moves your depletion age from 95 to 89") plus the numbers underneath; the other two just jump straight to the sensitivity ("tornado") chart or switch the main chart to the historical S&P 500 backtest and report its success rate. Uses whatever numbers are currently loaded, including the generic example data before you've entered your own
