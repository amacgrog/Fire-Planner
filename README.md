...
- **Guided setup wizard**: an optional 2-step modal, launched from the prominent "✨ Get started" button in the topbar, for getting a first projection running — step 1 asks for the basics (single/partnered, ages, income, retirement age, account balances, spending), step 2 offers a checklist of the features below that make the simulation more or less detailed (LTC, ACA, Roth conversion/RRSP meltdown ladder, mega backdoor Roth, bridge income, additional income, other loans, Guyton-Klinger guardrails). Every wizard field is a live proxy for the real sidebar control, so it never gets out of sync and can be safely skipped entirely
- **First-run banner** and **"Explore a question" menu**: sit above the two-column assumptions/results layout (not inside either side), so both are visible immediately on load and, on mobile, on both the Inputs and Dashboard tabs without switching — no need to flip tabs just to see them. The banner spells out that the loaded numbers are a generic example, not yours, and points at the two ways to fix that (the setup wizard, or importing a previously-exported scenario file); it clears itself the moment either happens, or the moment you edit any sidebar field directly. The question menu is a row of common questions — retire earlier or later, what if a spouse dies first, buy or rent, Roth conversions (RRSP meltdown in the Canada version), what's actually driving the results, what if returns are worse than expected — each pre-wired to an existing comparison rather than a blank slate. The first four run the current plan against 2-3 alternatives and answer with a plain-language sentence (e.g. "Retiring 3 years earlier costs you $340K and moves your depletion age from 95 to 89") plus the numbers underneath; the last two jump straight to the sensitivity ("tornado") chart or switch the main chart to the historical S&P 500 backtest and report its success rate, switching to the Dashboard tab first on mobile if needed. Uses whatever numbers are currently loaded, including the generic example data before you've entered your own
...
- **Collapsible sensitivity analysis ("tornado chart")**: shows how far projected end assets swing when one assumption moves at a time, holding everything else fixed, sorted by impact. Five assumptions (investment return, retirement age, household spending, inflation, bond return) are checked by default; five more (HYSA/cash rate, home appreciation, healthcare premium, long-term care cost, Social Security/CPP+OAS) are available via checkbox but off by default. Each swing is a fixed ± amount, e.g. "±2pp" (percentage points) or "±10%", not a probability distribution — a dashed line, labeled with its value, marks the current projection's end assets for comparison. If the current plan is projected to run out of money before the end of the modeled horizon, end assets would show $0 for every single bar (masking real differences between assumptions), so the chart automatically switches to measuring peak portfolio value instead — the dashed line is then labeled "Current (peak)" and a note under the chart explains the switch. Collapsed by default and only computed while expanded, so it never slows down the main dashboard
...
- **Cash-flow Sankey diagram** (collapsible, only computed while expanded): for a single selected year (a slider steps through the whole projection), shows where that year's money comes from (wages, Social Security/CPP+OAS, pension, traditional/RRIF withdrawal, taxable/brokerage withdrawal) on the left and where it goes (taxes, housing, healthcare, LTC, loan payments, discretionary spending) on the right, funneled through a single "pooled cash flow" hub in the middle — any remaining gap (Roth, cash, or home-equity draws; or a surplus) is shown as "From portfolio"/"To savings". The traditional and taxable/brokerage source amounts include whatever was drawn from the matching slice of the protected bond reserve (bondTraditional/bondTaxable), not just the equity sub-account, so a year that's still drawing bond-reserve money doesn't understate them. Hover (or focus via keyboard) any income source, hub, or spending category for its exact dollar amount
...
- **Mobile-responsive layout**: usable on a phone-width screen. Below 820px wide, an "Inputs / Dashboard" tab bar replaces the stacked one-long-scroll layout — Inputs shows the assumptions panel, Dashboard shows the results (metrics/chart/table/notes), and switching between them re-renders the dashboard so its charts always measure their container at the correct, visible width. Chart-view buttons also wrap instead of overflowing. Desktop is unaffected — the two-column layout there ignores the tabs entirely
